...
-- Modificar 
--   [TOTVS_FLUIG_PROD].[dbo].[ML001007]
--   [TOTVS_FLUIG_PROD].[dbo].[ML001018]
-- Por lo que corresponda en cada ambiente
--

--Se cambio funcion CONVERT por TRY_CONVERT para que no tire error el programa si hay una fecha que tiene mal el formato
...
SELECT fp.nroSolicitud,
		...
		TRY_CONVERT(VARCHAR(10), TRY_CONVERT(datetime, CAST(fp.fechaRelevamientoPerfil AS VARCHAR), 103), 120) AS fechaRelevamientoPerfilFiltro,
		...
		FROM [TOTVS_FLUIG_PROD].[dbo].[ML001007] fp1 WHERE fp1.id = fp.id ),1,1,'')AS accesosTelefonia,
...
ALTER VIEW [dbo].[V_RAF06_ALTA_EMPLEADO] AS 
	SELECT 
		...
		fp.fechaAlta,
		TRY_CONVERT(VARCHAR(10), TRY_CONVERT(datetime, CAST(fp.fechaAlta AS VARCHAR), 103), 120) AS fechaAltaFiltro,
		fp.fechaIngreso,
		...
		STUFF((SELECT
			...
			CASE WHEN fp1.senior like 'true' THEN '/Senior' ELSE '' END + 
			CASE WHEN fp1.noCorresponde like 'true' THEN '/NC' ELSE '' END 
		FROM [TOTVS_FLUIG_PROD].[dbo].[ML001018] fp1 WHERE fp1.id = fp.id ),1,1,'')AS seniority,
		fp.gerencia,
		fp.area,
		fp.sector,
		fp.subsector,
		...
		STUFF((SELECT
			...
			CASE WHEN fp1.salidaDDNInteracional like 'true' THEN '/DDI' ELSE '' END + 
			CASE WHEN fp1.salidaCelular like 'true' THEN '/Celular' ELSE '' END 
		FROM [TOTVS_FLUIG_PROD].[dbo].[ML001018] fp1 WHERE fp1.id = fp.id ),1,1,'')AS accesosTelefonia,
		CASE WHEN fp.tipoPosicion like 'puestoNuevo' THEN 'Puesto Nuevo' ELSE 'Reemplazo' END AS tipoPosicion,
		fp.nombreReemplazo,
		CASE WHEN fp.motivoReemplazo like 'renuncia' THEN 'Renuncia' 
			ELSE (CASE WHEN fp.motivoReemplazo like 'desvinculacion' THEN 'Desvinculación' 
		...
		STUFF((SELECT
			...
			CASE WHEN fp1.posgradoCurso like 'true' THEN '/Posgrado en Curso' ELSE '' END + 
			CASE WHEN fp1.posgradoGraduado like 'true' THEN '/Posgrado Graduado' ELSE '' END 
		FROM [TOTVS_FLUIG_PROD].[dbo].[ML001018] fp1 WHERE fp1.id = fp.id ),1,1,'') AS nivelEstudio,
		fp.carrera,
		fp.origenMatricula,
		fp.descExperienciaLaboralReq,
		fp.habilidadesNecesarias,
	 ...
	 FROM [TOTVS_FLUIG_PROD].[dbo].[ML001018] fp, 
		  [TOTVS_FLUIG_PROD].[dbo].DOCUMENTO d
		 WHERE fp.documentid = d.NR_DOCUMENTO
			 AND fp.version = d.NR_VERSAO
			 AND d.VERSAO_ATIVA = 1
