		...
		custom: function() {
			...
			$('#usuarioColaborador').arqzoom({
				...
				callback: function(res){
					...
					$('#lugarPago').val(res['lugarPago']).change();
...
//			$('.nivelEstudio').change();
		...
		bpm: {
			...
			"Adjuntar Documentación al Legajo": {
				task: [56],  
				custom: function() {
					...
					deshabilitarTodo();
					$('#documentacion').show();
					$('.documentacion :input').prop('disabled',false);
					
					$('#tablaDocumentacion').arqmasterdetail({
						buttonNewRow: '#btnNuevoDoc',
						buttonsDeleteRow: '.deleteRow',		
						...
						onCustomizeRow: function($tr, index){
							$('.docLink',$tr).click(function(){
								if($('.documentoPK',$tr).val()!=''){
				        			var documentoPK = $('.documentoPK',$tr).val().split('-');
				        			openDocument(documentoPK[0], documentoPK[1]);		        			
				        		}
				        	});
							
							$('.tipoDoc',$tr).arqzoom({
								datasetId: 'RAF06-CatalogoDocumentos',
								resultFields: ['nombre','documentoEstandar'],
								template: {
									row: [
									    { field: 'nombre'},
									],
								},
								filterValues: {'documentoEstandar':'NO'},
								searchField: 'nombre',
								displayKey: function(res){
									return res['nombre'];
								}
							...
							$('.documento',$tr).arqzoom({
								datasetId: 'raf-documentsIntoFolderRecursive',
								...
								filterValues: {'folderId': $('#carpetaLegajo').val()},
								searchField: 'documentDescription',
								displayKey: function(res){
									return res['documentDescription'];
								},
								callback: function(res){
						        	$('.documentoPK',$tr).val(res['documentPK.documentId']+'-'+res['documentPK.version']);
						        },
						        clean: function(res){
						        	$('.documentoPK',$tr).val('');
						        }
...
function cargarTablaDocumentacion(){
	...
	if($("#tablaDocumentacion tr:gt(1)").length == 0){
		...
		.done(function(dataset) {	
			if(dataset.values.length > 0) {
				for (var i = 0; i < dataset.values.length; i++) {
					var documentoEstandar = dataset.values[i]["documentoEstandar"];
				  	var tipoDocumento = dataset.values[i]["nombre"];

					var $row = $("#tablaDocumentacion").arqmasterdetail("addNewRow");
					$('.tipoDoc', $row).val(tipoDocumento).change();
					$('.tipoDoc', $row).arqzoom('disabled',true);
					$('.deleteRow', $row).hide();
				}
			}
		})
		.fail(function(jqXHR, textStatus, errorThrown) {
			alert('No se pudo cargar los Documentos.');
		});			
	}
}
...
//	$('tr', $('[id^="tablaDoc"]')).each(function(){
//		var $tr = $(this);
//		$('.docLink',$tr).click(function(){
//			if($('.docAdjPK',$tr).val()!=''){
//				var docAdjPK = $('.docAdjPK',$tr).val().split('-');
//				arqForms.openDocument(docAdjPK[0], docAdjPK[1]);					
//			}
//		});								
//	});
//};

function openDocument(WDNrDocto, WDNrVersao){
	var url = '/webdesk/streamcontrol/?companyId='+arqFormOpts.WKCompany+'&WDNrDocto=' + WDNrDocto + '&WDNrVersao=' + WDNrVersao;
