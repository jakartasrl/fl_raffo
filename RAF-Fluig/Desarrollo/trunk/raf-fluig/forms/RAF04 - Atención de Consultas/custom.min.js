...
var	arqForms = {
	...
	onEdit: function() {
		...
		$('#historicoObservaciones').html($('#historicoObservacionesInput').val());
		
		if(trim($('#historicoObservacionesInput').val()) != "" &&  $('#mostrarHistoricoObservaciones').val() == "1"){
				...
				workflow: {
					'Corregir Datos': {
						...
						custom: function() {
							...
							$('#aten_derivacion').prop('disabled', false);
					...
					'Revisar Consulta': {
						...
						custom: function() {
							...
							$('#aten_licenciante').prop('disabled',false);
							$('#aten_contieneEventoAdverso').prop('disabled',false);
							$('#aten_contieneEventoAdversoMotivo').prop('disabled',false);
							$('#aten_revobservacion').prop('disabled',false);
							$('#aten_conclusion').prop('disabled',false);
							$('#aten_confirma').prop('disabled', false);							
						...
						'otherwise': function() {
							$('#aten_confirma').prop('disabled', true);
						}
					},
					'Completar Consulta': {
						task: [5],  
						custom: function() {
							...
							$('#aten_resolucion').prop('disabled',false);
							$('#aten_motivoAtraso').prop('disabled',false);
							//$('#aten_derivacion').prop('disabled',false);
...
function getLinkSolicitudPadre(){
	...
	openedWin = window.open('/portal/p/'+companyID+'/pageworkflowview?app_ecm_workflowview_detailsProcessInstanceID=' + $('#padre_nroSolicitud').val());
	
	var timer = setInterval(function() {
		
	    if(openedWin.document.getElementById("workflowView-cardViewer") && 
	    		openedWin.document.getElementById("workflowView-cardViewer").contentWindow.arqForms &&
	    		openedWin.document.getElementById("workflowView-cardViewer").contentWindow.arqForms.registerListener) {  
	    	
	        openedWin.document.getElementById("workflowView-cardViewer").contentWindow.arqForms.listener = arqForms.afterWKRC_ModificarDatos;
	        clearInterval(timer);  
	    }  
	}, 500); 
