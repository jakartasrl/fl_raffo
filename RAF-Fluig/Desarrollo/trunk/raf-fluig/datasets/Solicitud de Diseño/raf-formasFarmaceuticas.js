 ...
 * Descripcion: devuelve el dataset raf-formasFarmaceuticas consultandolo directo a la tabla. 
 */
/*
 * Devuelve el dataset raf-formasFarmaceuticas consultandolo directo a la tabla.
 */
function createDataset(fields, constraints, sortFields) {
	...
	var where = arq.sql.whereHelper({
		...
		filtros: {
			...
			'descr': function(filtro) {
				return " AND descr LIKE '" + filtro.initialValue + "'";
			},
	...
	return arq.sql.sql2Dataset({
		...
		sql: " SELECT distinct cod, descr" + 
			 "   FROM " + getMetaLista("RAF07-FormasFarmaceuticas.async") +
			 " 	 WHERE " + where +
			 "  ORDER BY " + orderBy + ";",
		log: 1
	});
