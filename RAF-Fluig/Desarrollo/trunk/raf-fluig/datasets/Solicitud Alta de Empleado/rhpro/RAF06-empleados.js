/*
* Código: raf-empleados
* Descripción: Dataset que devuelve el listado de empleados.
*/
function createDataset(fields, constraints, sortFields) {
	...
	var where = arq.sql.whereHelper({
		...
		filtros: {
			'nombreCompleto': function(filtro) {
				return " AND nombreCompleto LIKE '%" + filtro.initialValue + "%'";
			},
		}
	});
	...
	return arq.sql.sql2Dataset({
		...
		sql: " SELECT  matricula, nombreCompleto, puesto, categoria, estado, nombre, apellido, tipoDocumento, nroDocumento, cuil,fechaNacimiento, genero, estadoCivil, nacionalidad, empresa, lugarPago, domicilio, nombreCompleto + ' | ' + domicilio as nombreDomicilio " + 
			 "   FROM Z_RAF_EMPLEADO " +
			 " 	WHERE " + where +
