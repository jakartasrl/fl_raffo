 ...
 * Descripcion: Atencion de Consultas para Grupo de Derivación.
 */
function createDataset(fields, constraints, sortFields) { 

	// Define los campos para ordenacion 
	var sortingFields = new Array("aten_nroSolicitud");
	...
	for (var i=0; i<constraints.length; i++){
		log.info("raf-atencion-consultas-AMED - Constraint para busqueda de solicitudes: " + constraints[i].fieldName + " desde: " + constraints[i].initialValue + " hasta: " + constraints[i].finalValue);
	}
	
	var datasetAtenciones = DatasetFactory.getDataset("raf-atencion-consultas", null, constraints, sortingFields);
	
	log.info("raf-atencion-consultas-AMED - Cantidad de solicitudes anteriores encontradas: " + datasetAtenciones.rowsCount);
