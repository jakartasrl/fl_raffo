var	arqForms = {
	...
	onEdit: function() {
		...
		$('#referenciaControl').ecmzoom({
			...
			dataFields: [
				{id: 'descripcion', value: 'Descripci�n'}
			],  
		...
		$('#rubro').ecmzoom({
			...
			dataFields: [
				{id: 'rub_codigo', value: 'C�digo'},
				{id: 'rub_descripcion', value: 'Descripci�n'}
			],  
			filterValues: [{ id: 'metadata%23active', value: true}], 
		...
		$('#frecuenciaFacturacion').ecmzoom({
			...
			dataFields: [
				{id: 'fre_codigo', value: 'C�digo'},
				{id: 'fre_descripcion', value: 'Descripci�n'}
			],  
			filterValues: [{ id: 'metadata%23active', value: true}], 
			resultFields: ['fre_descripcion'],
			callback: function(res){
		...
		$('#motivoAdjudicacion').ecmzoom({
			...
			dataFields: [
				{id: 'mot_codigo', value: 'C�digo'},
				{id: 'mot_descripcion', value: 'Descripci�n'}
			],  
			filterValues: [{ id: 'metadata%23active', value: true}], 
			resultFields: ['mot_descripcion'],
			callback: function(res){
		...
		$('#referenciaControl').ecmzoom({
			...
			dataFields: [
				{id: 'referenciaControl', value: 'C�digo'}
			],  
			filterValues: [{ id: 'metadata%23active', value: true}], 
		...
		for(var i=1; i<=5; i++){
			$('#provs_codDesc'+i).ecmzoom({
				datasetId: 'raf-proveedores',
				dataFields: [
					{id: 'codigoProveedor', value: 'C�digo'},
					{id: 'razonSocialProveedor', value: 'Raz�n Social'},
					{id: 'codigoFormaPago', value: 'Forma Pago C�digo'},
					{id: 'descripcionFormaPago', value: 'Forma Pago Descripci�n'},
		...
		$("#tablaDetalleProducto").ecmmasterdetail({
			...
			onCustomizeRow: function($tr) {	
				...
				if ($(".noCodificado", $tr).prop('checked')){
					...
					$(".det_prod_descripcion", $tr).attr('placeholder',"DESCRIPCI�N");
				...
				$('.det_prod_buscar', $tr).ecmzoom({
					...
					dataFields: [
						{id: 'codigoProducto', value: 'C�digo'},
						{id: 'descripcionProducto', value: 'Descripci�n'},
						{id: 'estadoProducto', value: 'Descripci�n'},
						{id: 'codigoUnidadMedida', value: 'Descripci�n'},
						{id: 'descUnidadMedida', value: 'Descripci�n'}
					],  
					filterValues: [{ id: 'metadata%23active', value: true}], 
		...
		if($('input:radio[name=imputacionContable]').is(':checked')){
	...
	if($("#provs_estado" + provedorSeleccionadoIndex).val() == "temporal"){
		...
		$("#tablaDetalleProducto tr:gt(1)").each(function (index, value){
			...
			if(!$(this).parent().is("tfoot")){
				...
				$('.prod_moneda',$row).val($(this).find("input.det_prod_moneda").val());
